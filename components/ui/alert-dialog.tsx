...
function AlertDialogOverlay({
  ...
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50 bg-[var(--surface-dialog-overlay)]",
        className
      )}
      onClick={(e) => {
        e.stopPropagation();
...
function AlertDialogMedia({
  ...
  return (
    <div
      ...
      className={cn(
        "bg-muted mb-2 inline-flex size-16 items-center justify-center rounded-[var(--radius-md)] sm:group-data-[size=default]/alert-dialog-content:row-span-2 *:[svg:not([class*='size-'])]:size-8",
        className
      )}
      {...props}
    />
